...
document.getElementById("api-key-input").addEventListener("keydown", (e) => {
    if (e.key === "Enter") document.getElementById("save-api-key-btn").click();
});
...
function addMessage(sender, text, timestamp = null, save = true) {
    ...
    if (save) saveMessageToStorage(sender, text, ts.toISOString());
}
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (code === 401 || code === 403) {
                showErrorMessage("API key invalid or unauthorized. Click 'Change Key' to update it.");
            } else if (code === 404) {
...
stopBtn.addEventListener("click", () => {
    ...
    if (currentTyping) { currentTyping.cancel(); currentTyping = null; }
});
...
exportBtn.addEventListener("click", () => {
    const saved = JSON.parse(localStorage.getItem("droidchat_conversations") || "{}");
    const messages = saved[currentConversation] || [];
    if (messages.length === 0) { alert("Nothing to export!"); return; }
